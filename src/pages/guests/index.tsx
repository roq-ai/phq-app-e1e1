...
export function GuestListPage(props: GuestListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getGuests({
        relations: ['user', 'organization', 'answer.count', 'phq_9_question.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['status.contains', 'medication.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.registration_date ? format(parseISO(record?.registration_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    {
      id: 'last_login',
      header: 'Last Login',
    ...
    hasAccess('answer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('phq_9_question', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'phq_9_question',
          header: 'Phq 9 Question',
          accessorKey: 'phq_9_question',
          cell: ({ row: { original: record } }: any) => record?._count?.phq_9_question,
        }
      : null,
