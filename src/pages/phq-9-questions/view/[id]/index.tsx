...
import { getPhq9QuestionById } from 'apiSdk/phq-9-questions';
import { Phq9QuestionInterface } from 'interfaces/phq-9-question';
import { AnswerListPage } from 'pages/answers';

function Phq9QuestionViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<Phq9QuestionInterface>(
    () => (id ? `/phq-9-questions/${id}` : null),
    () =>
      getPhq9QuestionById(id, {
        relations: ['guest', 'healthcare_provider'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('phq_9_question', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/phq-9-questions/edit/${id}`} passHref legacyBehavior>
                    <Button
